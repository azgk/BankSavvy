...
class EOM(DateRange):
  def __init__(self, year, month, EOM_month_dir, exp_acct_info, depo_acct_info):
    ...
    self.tally_exp_accts()
    self.tally_depo_accts()
    self.summary["Expenses"] = self.get_total(self.summary["Expenses"], "Expenses")
    self.summary["Deposits"] = self.get_total(self.summary["Deposits"], "Deposits")
    self.print_acct_details()

  def modify_df(self, df, date_col, amount_col):
    """
    Filter df by date. Also, convert dollar amount from str to float if needed.
    ...
    if amount_is_str:
      for index, row in modified_df.iterrows():
        try:
          modified_df.at[index, amount_col] = 0 - locale.atof(row[amount_col][1:])  # When "$" is used, Expense is often shown as positive in bank data. Thus making it negative so that it matches the format/pattern of other bank data.
        except TypeError:  # When the cell is blank (NaN).
          modified_df.at[index, amount_col] = 0.0
